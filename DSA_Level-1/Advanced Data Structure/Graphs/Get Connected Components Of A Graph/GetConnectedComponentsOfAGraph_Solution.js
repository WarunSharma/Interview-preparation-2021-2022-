...
function getConnectedComponentsHelper(graph,visited,src,comp){
    visited[src]=true;
    comp.push(src);
    let edges=graph[src];
    ...
    for(let edge of edges){
        if(visited[edge.v2]==false){
            getConnectedComponentsHelper(graph,visited,edge.v2,comp);
        }
    }
}

function getConnectedComponents(graph,visited,vtces,comps){
    for(let i=0;i<vtces;++i){
        let comp=[];
        if(visited[i]==false){
            getConnectedComponentsHelper(graph,visited,i,comp);
            comps.push(comp);
        }
    }
}
...
let comps=[];
getConnectedComponents(result,visited,result.length,comps);
console.log(comps);