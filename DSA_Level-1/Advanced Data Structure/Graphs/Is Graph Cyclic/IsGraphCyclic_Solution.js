...
function createGraph(){
    ...
    graph[3].push(new Edge(3,4,10));
    graph[3].push(new Edge(3,2,10));

    graph[4].push(new Edge(4,3,10));
    //graph[4].push(new Edge(4,5,3));
    graph[4].push(new Edge(4,6,10));
    graph[4].push(new Edge(4,5,10));

     graph[5].push(new Edge(5,4,10));
     //graph[5].push(new Edge(5,6,3));
     graph[5].push(new Edge(5,6,10));

      graph[6].push(new Edge(6,4,10));
      graph[6].push(new Edge(6,5,10));
...
function isGraphConnected(graph,visited,src){
    isGraphConnectedHelper(graph,visited,src);
...
function isGraphConnectedHelper(graph,visited,src){
    visited[src]=true;
    let edges=graph[src];
    for(let i=0;i<edges.length;++i){
        let edge=edges[i];
        if(visited[edge.v2]==false){
            isGraphConnectedHelper(graph,visited,edge.v2);
        }
    }
}

function isGraphCyclicHelper(graph,visited,src){
    let queue=new Queue();
    queue.add(src);

    while(queue.size()>0){
        let rem=queue.remove();
...
function isGraphCyclic(graph,visited){

    for(let i=0;i<visited.length;++i){
        if(visited[i]==false){
            let cyclic=isGraphCyclicHelper(graph,visited,i);
...
//console.log(isGraphConnected(result,visited,0));

console.log(isGraphCyclic(result,visited));