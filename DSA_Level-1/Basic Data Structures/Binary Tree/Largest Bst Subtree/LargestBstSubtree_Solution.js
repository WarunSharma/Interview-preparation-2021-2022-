...
class BST{
    constructor(min,max,isBST,size,node){
        this.min=min;
        this.max=max;
        this.isBST=isBST;
        this.size=size;
...
function isBinarySearchTree(node){
    ...
    if(node==null){
        return new BST(Number.MAX_SAFE_INTEGER,Number.MIN_SAFE_INTEGER,true,0,null);
    }
    ...
    if(res.isBST){
        res.size=1+leftBST.size+rightBST.size;
        res.node=node.data;
    }
    else{
        res.size=Math.max(leftBST.size,rightBST.size);
        if(leftBST>rightBST){
            res.node=leftBST.node;
        }
        else{
            res.node=rightBST.node;
        }
    }
...
root= construct([50, 25, 12, null, null, 37, null, null, 75, 62, null, null, 87, null, null])
// display(root);
...
console.log(isBinarySearchTree(root).node+'@'+isBinarySearchTree(root).size);