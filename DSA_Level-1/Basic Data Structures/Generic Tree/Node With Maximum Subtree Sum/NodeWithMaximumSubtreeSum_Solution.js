...
function maxSumSubTree(node){
    let max=node.data;
    for(let child of node.children){
        let sum=maxSumSubTree(child);
        max+=sum;
    }
...
let root=construct([10, 20, -50, -1, 60, -1, -1, 30, -70, -1, 80, -1, 90, -1, -1, 40, -100, -1, -1, -1]);
// display(root);
//console.log(sizeOfGenericTree(root));
//console.log(maximum(root));
// console.log(height(root));
maxSumSubTree(root);
console.log(`${msn.data}@${maxSum}`);
